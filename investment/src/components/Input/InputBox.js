...
const InputBox = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    const updatedData = {
      currentSavings: event.target.querySelector("#current-savings").value,
      yearlyContribution: event.target.querySelector("#yearly-contribution")
        .value,
      expectedReturn: event.target.querySelector("#expected-return").value,
  ...
  return (
    <form className={styles.form} onSubmit={submitHandler}>
      ...
      <InputAction resetHandler={resetHandler} />
    </form>
  );
};
